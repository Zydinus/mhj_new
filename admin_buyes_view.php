        ...
        <form method="get">
          <div class="col-lg-2 col-lg-offset-5">
            ...
            <div class="form-group text-right">
              <a href="admin_buyes_view.php" class="btn btn-default" role="button">
      ...
      <div class="row">
        ...
        <div class="col-lg-12">
          ...
          <table class="table table-hover table-striped table-condensed">
            ...
            <tbody>
              <?php
              function makeInfoButton($buy) {
                $button = "<a role='button' class='btn btn-info btn-sm' ";
                $button .= "href='admin_buy_view.php?id=$buy[buy_id]'>";
              ...
              function makeDeleteButton($buy) {
                $button = "<button type='button' class='btn btn-danger btn-sm' ";
                $button .= "onclick='javascript:deleteBuy($buy[buy_id])'>";
              ...
              $buyes = getBuyesWithTotal($conn, $begin_date, $end_date);
              foreach ($buyes as $buy) {
                ?>
                <tr class="<?= $buy["is_price_edited"]>0 ? "info": ""?>">
                  <td><?= $buy["customer_name"]?></td>
                  <td><?= $buy["contact_name"]?></td>
                  <td><?= $buy["buy_created_at"]?></td>
                  <td><?= $buy["total"]?></td>
                  <td><?= $buy["user_name"]?></td>
                  <td><?= makeInfoButton($buy) ?> <?= makeDeleteButton($buy) ?></td>
                </tr>
                <?php
              }
              ?>
